<x-jet-action-section>
    <x-slot name="title">
        <span style="color: rgb(0, 0, 0);">{{ __('Autenticación de dos factores') }}</span>
    </x-slot>

    <x-slot name="description">
        <span style="color: rgb(0, 0, 0);">{{ __('Agregue seguridad adicional a su cuenta mediante la autenticación de dos factores.') }}</span>
    </x-slot>

    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                {{ __('Se ha habilitado la autenticación de dos factores.') }}
            @else
